...
For Home Assistant running in Docker, find the TrueNAS host path mounted into
...
command_line:
  - sensor:
      ...
      command: >-
        python3 -c "import json, urllib.request; data=json.load(urllib.request.urlopen('http://192.168.1.156:1000/', timeout=10)); print(json.dumps({'count': len(data), 'prices': data}))"
...
```bash
VITE_PEAK_RATE_URL=http://192.168.1.156:1000/
```
...
```yaml
rest:
  - resource: http://192.168.1.156:7070/api/tariff/solar
    scan_interval: 900
    sensor:
...
```bash
VITE_EVCC_URL=http://192.168.1.156:7070
VITE_EVCC_SOLAR_FORECAST_URL=http://192.168.1.156:7070/api/tariff/solar
```
...
command_line:
  - sensor:
      ...
      command: >-
        python3 -c "import json, urllib.request; data=json.load(urllib.request.urlopen('http://192.168.1.156:7070/api/sessions', timeout=10)); print(json.dumps({'count': len(data), 'sessions': data[:25]}))"
      value_template: "{{ value_json.count }}"
      json_attributes:
        - sessions
      scan_interval: 300
...
```bash
VITE_EVCC_SESSIONS_URL=http://192.168.1.156:7070/api/sessions
```
